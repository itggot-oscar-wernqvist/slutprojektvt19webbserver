...
def prevoius_post_vote(post_id, user_id)
    db = SQLite3::Database.new("db/reddit.db")
    result = db.execute("SELECT post_upvotes.post_id, post_upvotes.user_id FROM post_upvotes WHERE post_upvotes.user_id = ? AND post_upvotes.post_id = ?", user_id, post_id) 
    if result.length > 0
        return true
    else
        return false
...
def vote_post(post_id, user_id, value)
    db = SQLite3::Database.new("db/reddit.db")
    db.execute("INSERT INTO post_upvotes (post_id, user_id) VALUES (?,?)", post_id, user_id)
    upvote_count = db.execute("SELECT posts.upvote_count FROM posts WHERE posts.post_id = ?", post_id)[0][0].to_i
    db.execute("UPDATE posts SET upvote_count = ? WHERE post_id = ?", upvote_count + value, post_id)
end